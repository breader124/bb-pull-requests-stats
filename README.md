...
```bash
bb-pull-requests-stats$ sam build
bb-pull-requests-stats$ sam deploy --guided
```
...
```bash
bb-pull-requests-stats$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
bb-pull-requests-stats$ sam local start-api
bb-pull-requests-stats$ curl http://localhost:3000/
```
...
```bash
bb-pull-requests-stats$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
bb-pull-requests-stats$ cd HelloWorldFunction
